...
public class ModConfig {
    ...
    private static final String EXHAUSTION_SYNC_THRESHOLD_COMMENT = "The maximum difference between the server's value for exhaustion and the client's before the value is syncronized from the server to the client.\n"
    ...
    private static final String ALWAYS_SHOW_FOOD_VALUES_TOOLTIP_COMMENT = "If true, shows the hunger and saturation values of food in its tooltip automatically (without needing to hold SHIFT)";
    ...
    private static final String SHOW_FOOD_VALUES_OVERLAY_COMMENT = "If true, shows the hunger (and saturation if "
            + SHOW_SATURATION_OVERLAY_NAME
            + " is true) that would be restored by food you are currently holding";
    ...
    public static void sync() {
         ...
         * SERVER
         */
        config.getCategory(CATEGORY_SERVER).setLanguageKey(LANG_PREFIX + CATEGORY_SERVER)
                .setComment(CATEGORY_SERVER_COMMENT);

        EXHAUSTION_SYNC_THRESHOLD = config
                .get(
                        CATEGORY_SERVER,
                        EXHAUSTION_SYNC_THRESHOLD_NAME,
                        EXHAUSTION_SYNC_THRESHOLD_DEFAULT,
                        EXHAUSTION_SYNC_THRESHOLD_COMMENT)
                .setLanguageKey(LANG_PREFIX + EXHAUSTION_SYNC_THRESHOLD_NAME).getDouble();

        /*
         * CLIENT
         */
        config.getCategory(CATEGORY_CLIENT).setLanguageKey(LANG_PREFIX + CATEGORY_CLIENT)
                .setComment(CATEGORY_CLIENT_COMMENT);

        // rename overlay to underlay
        boolean foodExhaustionOverlayValue = config.get(CATEGORY_CLIENT, SHOW_FOOD_EXHAUSTION_OVERLAY_NAME, true)
                .getBoolean();
        config.getCategory(CATEGORY_CLIENT).remove(SHOW_FOOD_EXHAUSTION_OVERLAY_NAME);

        SHOW_FOOD_VALUES_IN_TOOLTIP = config
                .get(CATEGORY_CLIENT, SHOW_FOOD_VALUES_IN_TOOLTIP_NAME, true, SHOW_FOOD_VALUES_IN_TOOLTIP_COMMENT)
                .setLanguageKey(LANG_PREFIX + SHOW_FOOD_VALUES_IN_TOOLTIP_NAME).getBoolean();
        ALWAYS_SHOW_FOOD_VALUES_TOOLTIP = config
                .get(
                        CATEGORY_CLIENT,
                        ALWAYS_SHOW_FOOD_VALUES_TOOLTIP_NAME,
                        false,
                        ALWAYS_SHOW_FOOD_VALUES_TOOLTIP_COMMENT)
                .setLanguageKey(LANG_PREFIX + ALWAYS_SHOW_FOOD_VALUES_TOOLTIP_NAME).getBoolean();
        SHOW_SATURATION_OVERLAY = config
                .get(CATEGORY_CLIENT, SHOW_SATURATION_OVERLAY_NAME, true, SHOW_SATURATION_OVERLAY_COMMENT)
                .setLanguageKey(LANG_PREFIX + SHOW_SATURATION_OVERLAY_NAME).getBoolean();
        SHOW_FOOD_VALUES_OVERLAY = config
                .get(CATEGORY_CLIENT, SHOW_FOOD_VALUES_OVERLAY_NAME, true, SHOW_FOOD_VALUES_OVERLAY_COMMENT)
                .setLanguageKey(LANG_PREFIX + SHOW_FOOD_VALUES_OVERLAY_NAME).getBoolean();
        SHOW_FOOD_EXHAUSTION_UNDERLAY = config
                .get(
                        CATEGORY_CLIENT,
                        SHOW_FOOD_EXHAUSTION_UNDERLAY_NAME,
                        foodExhaustionOverlayValue,
                        SHOW_FOOD_EXHAUSTION_UNDERLAY_COMMENT)
                .setLanguageKey(LANG_PREFIX + SHOW_FOOD_EXHAUSTION_UNDERLAY_NAME).getBoolean();
        SHOW_FOOD_DEBUG_INFO = config
                .get(CATEGORY_CLIENT, SHOW_FOOD_DEBUG_INFO_NAME, true, SHOW_FOOD_DEBUG_INFO_COMMENT)
                .setLanguageKey(LANG_PREFIX + SHOW_FOOD_DEBUG_INFO_NAME).getBoolean();
