				...
				if (player.inventory.getItemStack() == null && hoveredStack != null && AppleCoreAPI.accessor.isFood(hoveredStack)) {
					...
					for (int i = 0; i < barsNeeded * 2; i += 2) {
						...
						if (modifiedFoodValues.hunger > i) {
							curScreen.drawTexturedModalRect(x, y, modifiedFoodValues.hunger - 1 == i ? 61 : 52, 27, 9, 9);
						}
					}
					...
					for (int i = 0; i < saturationBarsNeeded * 2; i += 2) {
						...
						curScreen.drawTexturedModalRect(x * 4 / 3, y * 4 / 3, effectiveSaturationOfBar >= 1 ? 27 : effectiveSaturationOfBar > 0.5 ? 18 : effectiveSaturationOfBar > 0.25 ? 9 : effectiveSaturationOfBar > 0 ? 0 : 36, modifiedSaturationIncrement >= 0 ? 0 : 9, 9, 9);
