...
public class BlockCropsMixin extends BlockBush {
    ...
    public void updateTick(World world, int blockX, int blockY, int blockZ, Random random) {
        ...
        if (allowGrowthResult == Event.Result.ALLOW || (allowGrowthResult == Event.Result.DEFAULT && world.getBlockLightValue(blockX, blockY + 1, blockZ) >= 9)) {
            int metadata = world.getBlockMetadata(blockX, blockY, blockZ);
            if (metadata < 7) {
                float f = this.func_149864_n(world, blockX, blockY, blockZ);

                if (allowGrowthResult == Event.Result.ALLOW || (allowGrowthResult == Event.Result.DEFAULT && random.nextInt((int) (25.0F / f) + 1) == 0)) {
